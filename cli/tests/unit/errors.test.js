...
import { PreflightError, ProcessingError, SystemError, isArenaError, } from '../../src/errors/index.js';
import { formatErrorWithHelp } from '../../src/errors/formatter.js';
describe('Error Classes', () => {
    describe('PreflightError', () => {
        it('should create preflight error with code and message', () => {
            const error = new PreflightError('VIDEO_NOT_FOUND', 'Video file not found', 'Check the file path');
        ...
        it('should support optional docs URL', () => {
            const error = new PreflightError('API_KEY_MISSING', 'API key not found', 'Set OPENAI_API_KEY', 'https://docs.arena.com/api-key');
            expect(error.docsUrl).toBe('https://docs.arena.com/api-key');
        });
    });
    describe('ProcessingError', () => {
        it('should create processing error', () => {
            const error = new ProcessingError('TRANSCRIPTION_FAILED', 'Failed to transcribe audio', 'Check your API key');
...
describe('Error Formatter', () => {
    describe('formatErrorWithHelp', () => {
        it('should format preflight error', () => {
            const error = new PreflightError('VIDEO_NOT_FOUND', 'Video file not found: video.mp4', 'Check the file path and try again');
        ...
        it('should format error with docs URL', () => {
            const error = new PreflightError('API_KEY_MISSING', 'OpenAI API key not set', 'Set OPENAI_API_KEY environment variable', 'https://platform.openai.com/api-keys');
            const formatted = formatErrorWithHelp(error);
        ...
        it('should handle processing errors', () => {
            const error = new ProcessingError('PROCESSING_FAILED', 'Failed to generate clips', 'Check the error message above');
            const formatted = formatErrorWithHelp(error);
            expect(formatted).toContain('✗'); // Error symbol
        ...
        it('should handle system errors', () => {
            const error = new SystemError('INTERRUPTED', 'Process was interrupted by user', 'This is normal if you pressed Ctrl+C');
            const formatted = formatErrorWithHelp(error);
            expect(formatted).toContain('✗'); // Error symbol
